...
export class AnatomyViewer {
  ...
  /** Re-labels the currently mounted organ's hotspots (e.g. on a language
   *  switch) without the teardown/reload `setOrgan` does — the model, pivot,
   *  and meshes are unchanged, only the hotspot label/detail text is new. */
  relabelHotspots(hotspots: Hotspot[]) {
    if (!this.organ) return;
    this.select(null);
    this.hotspots.attach(this.organ.pivot, hotspots, this.organ.meshes);
    this.hotspots.setPixelSize(DOT_PIXELS, this.height, CAMERA_FOV);
    this.dirty = true;
  }
